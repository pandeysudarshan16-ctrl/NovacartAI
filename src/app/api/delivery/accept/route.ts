...
export async function PUT(req: NextRequest) {
  try {
    ...
    const delivery = await deliveryService.acceptDelivery(decoded.id, parsed.data.orderId);

    return handleApiSuccess(delivery, "Package delivery accepted successfully");
  } catch (error) {
